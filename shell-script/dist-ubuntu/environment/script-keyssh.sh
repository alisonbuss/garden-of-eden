...
function ScriptKeySSH {
    ...
    local     comment=$(echo ${PARAM_JSON} | $RUN_JQ -r '.comment');
    local passwordKey=$(echo ${PARAM_JSON} | $RUN_JQ -r '.passwordKey');
    local     pathKey=$(echo ${PARAM_JSON} | $RUN_JQ -r '.pathKey');
    local     nameKey=$(echo ${PARAM_JSON} | $RUN_JQ -r '.nameKey');

    # @descr: Função para Gerar chave SSH.
    __create() {
        util.print.out '%s\n' "Iniciando criação da chave SSH na maquina..."; 

        # verifica se tem a pasta, caso não tenha é criada uma pasta.
        if [ ! -d "$pathKey" ] ; then
            mkdir -p "${pathKey}";
            sudo chmod -R 760 "${pathKey}";
    ...
    __recreate() {
        util.print.out '%s\n' "Iniciando a recriação da chave SSH na maquina..."; 
